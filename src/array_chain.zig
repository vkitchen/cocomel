...
pub const ArrayChain = struct {
    ...
    fn ensureChunk(self: *Self, allocator: std.mem.Allocator) !void {
        ...
        if (chunk == null) {
            ...
            chunk.?.items = try allocator.alloc(u8, self.next_chunk_size);
            chunk.?.items.len = 0;
    ...
    pub fn ensureUnusedCapacity(self: *Self, allocator: std.mem.Allocator, additional_count: usize) !void {
        ...
        if (chunk.items.len + additional_count >= chunk.capacity) {
            ...
            nextChunk.items = try allocator.alloc(u8, self.next_chunk_size);
    ...
    pub fn append(self: *Self, allocator: std.mem.Allocator, item: u8) !void {
