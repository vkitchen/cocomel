    ...
    pub fn insert(self: *Self, key: []const u8, doc_id: u32) !void {
        ...
        while (self.store[i] != null) {
            ...
            if (std.mem.eql(u8, postings.term, key)) {
                ...
                try postings.ids.ensureUnusedCapacity(5);
                postings.ids.items.len += vbyte.store(postings.ids.items[postings.ids.items.len..], doc_id - postings.id);
                postings.id = doc_id;
        ...
        postings.ids = try std.ArrayList(u8).initCapacity(self.allocator, 8);
        postings.ids.items.len += vbyte.store(postings.ids.items[0..], doc_id - postings.id);
        postings.id = doc_id;
